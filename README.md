...
**Codepoints outside of Plane 0 do not display correctly using the C/C++/Java source code above. For codepoints which are 5 or 6 digits in length, use curly brackets around the digits. Example: The Unicode Codepoint for :snail: 'SNAIL' is U+1F40C, and will not display correctly unless written as `\u{1F40C}` or `\uD83D\uDC0C`. For more information, see '[Why do some of the language scripts look like they are a repeated character, but are also paired with a latin number or letter in my project?](https://github.com/SheilaRT/Azure-Alphant#why-do-some-of-the-language-scripts-look-like-they-are-a-repeated-character-but-are-also-paired-with-a-latin-number-or-letter-in-my-project)'.
...
You may send them a link to the Azure Alphant 'About' page, either as a text link (emails, texts, comments) or as a banner (blogs, articles). Feel free to tweak the height and width of the banner down from the default maximum size in order to fit your needs.
